...
def get_args():
    ...
    parser.add_argument("--flash_attn", action="store_true", default=False, help="Whether to use flash attention. Default to False")
...
def main():
    ...
    elif args.is_custom_model:
        config = {
            "model_builder": AutoModel.from_pretrained,
            "pipeline_builder": CustomLeftPadRewardBenchPipeline, #### CustomRightPadRewardBenchPipeline
            "quantized": False,
            ...
            if model_type == "Custom Classifier":
                if not args.is_custom_model:
                    ...
                    if args.is_general_preference:
                        if args.value_head_dim == 2 and not hasattr(reward_pipe.model, 'prompt_head'):
                            [
                                results.append(1) if chosen[0] * rejected[1] - chosen[1] * rejected[0] > 0 else results.append(0)
                                for chosen, rejected in zip(score_chosen_batch, score_rejected_batch)
                            ]
                        elif not hasattr(reward_pipe.model, 'prompt_head'):
                            result = generate_high_dim_result(args.value_head_dim, chosen_reward, reject_reward)
                            ...
                            result = generate_high_dim_result_with_prompt(reward_pipe.model, args.value_head_dim, chosen_reward, reject_reward, prompt_hidden_state)
                            result_batch = result.float().cpu().detach().numpy().tolist()
                            [
                                results.append(1) if result > 0 else results.append(0)
                                for result in result_batch
